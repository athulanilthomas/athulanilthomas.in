// @ts-nocheck — internal WebGL simulation classes use loose typing intentionally
import * as THREE from 'three'

export interface LiquidEtherInstance {
...
export function createLiquidEther(
  ...
  class MouseClass {
    ...
    diff = new THREE.Vector2(); timer = null; container = null
    ...
    init(c) {
      ...
      c.addEventListener('mouseenter', this._onMouseEnter)
  ...
  class AutoDriver {
    constructor(mouse, manager, o) {
      ...
      this.enabled = o.enabled; this.speed = o.speed
  ...
  // --- ShaderPass ---
  class ShaderPass {
    constructor(p) { this.props = p || {}; this.uniforms = this.props.material?.uniforms }
    init() {
  ...
  class Advection extends ShaderPass {
    ...
    createBoundary() {
      ...
      this.line = new THREE.LineSegments(bg, bm); this.scene.add(this.line)
    }
    update(a) {
      const { dt, isBounce, BFECC } = a || {}
  ...
  class ExternalForce extends ShaderPass {
    constructor(sp) { super({ output: sp.dst }); this._init(sp) }
    _init(sp) {
      super.init()
    ...
    update(p) {
      ...
      const cs = p.cellScale || { x: 1, y: 1 }
      const cz = p.cursor_size || 0
      const csX = cz * cs.x, csY = cz * cs.y
      ...
      u.force.value.set(forceX, forceY); u.center.value.set(cX, cY); u.scale.value.set(cz, cz)
      super.update()
    }
  }
  ...
  class Viscous extends ShaderPass {
    ...
    update(a) {
      const { viscous, iterations, dt } = a || {}
      let fi, fo
      if (typeof viscous === 'number') this.uniforms.v.value = viscous
      for (let i = 0; i < (iterations ?? 0); i++) {
        if (i % 2 === 0) { fi = this.props.output0; fo = this.props.output1 }
        else { fi = this.props.output1; fo = this.props.output0 }
        this.uniforms.velocity_new.value = fi.texture; this.props.output = fo
        if (typeof dt === 'number') this.uniforms.dt.value = dt
        super.update()
      }
      return fo
  ...
  class Poisson extends ShaderPass {
    ...
    update(a) {
      const { iterations } = a || {}; let pi, po
      for (let i = 0; i < (iterations ?? 0); i++) {
        if (i % 2 === 0) { pi = this.props.output0; po = this.props.output1 }
        else { pi = this.props.output1; po = this.props.output0 }
  ...
  class Simulation {
    ...
    createAllFBO() {
      ...
      for (const key in this.fbos) this.fbos[key] = new THREE.WebGLRenderTarget(this.fboSize.x, this.fboSize.y, o)
    }
    createShaderPass() {
  ...
  class WebGLManager {
    constructor(p) {
      this.props = p; this.running = false; this.lastUserInteraction = performance.now()
      ...
      this._onVisibility = () => {
        ...
        else if (callbacks.getIsVisible()) this.start()
      }
    ...
    loop() { if (!this.running) return; this.render(); callbacks.onRafId(requestAnimationFrame(this._loop)) }
